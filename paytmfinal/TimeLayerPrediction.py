...
def parent(userid):#Parameter Time Period and Discount Price
   while True:
      #newpid = os.fork()
      import pandas as pd
      import numpy as np
      ...
      sample=dataset.iloc[userid:userid+1,1:-1].values
      ...
      return(int((datetime.now()+timedelta(days=Waiting_block*3)).day))*60*60*24
